...
- As mensagens de notificação estarão também em arquivos externos `avisoNCob1.html` e `avisoNCob2.html`.
  ...
  - Quando o status do trabalho for ‘Enviado para Aceite’, verifique se a descrição está correta e corresponde à atividade.
  - Se houver algum erro, informe ao servidor e ao supervisor.

  #### Objetivo

  Verificar se o campo Descrição está correto e corresponde à atividade quando o trabalho está ‘Enviado para Aceite’. Caso haja algum erro, informe ao servidor e ao supervisor.

  #### Passos

  1. Obter a lista de trabalhos com status ‘Enviado para Aceite’ do banco de dados usando uma consulta SQL. Utilize a função `pontalina('SELECT DISTINCT [pactoTrabalhoId] FROM [ProgramaGestao].[VW_PlanoTrabalhoAUDIN] WHERE [SituacaoPactoTrabalho] = 'Enviado para Aceite' and descricao like '%<demanda>%%</demanda>%')`.
  2. Para cada pacto idTrabalho:
    1. Obter todas as atividades desta pactoTrabalhoId usando a função `pontalina('SELECT [NomeServidor], [pactoTrabalhoId], [titulo], [descricao] FROM [ProgramaGestao].[VW_PlanoTrabalhoAUDIN] WHERE [pactoTrabalhoId] = <idTrabalho>')`.
    2. Para cada atividade:
      1. Verificar se o campo Descrição está correto.
        1. Se estiver correto, ignorar e passar para a próxima atividade.
        2. Se não estiver correto, adicione o erro à lista de erros.
      2. Após todos terem sido lidos, envie um email ao servidor com a lista de erros.
      3. Se não houver erros, verifique se [titulo] corresponde à descrição informada pelo servidor.
      4. Se não coincidir, envie um email ao servidor e ao supervisor informando que o título não coincide com a descrição.
      5. Se coincidir enviar email para supervisor.
